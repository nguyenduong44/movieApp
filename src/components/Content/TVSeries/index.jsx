...
function TVSeries() {
  ...
  const fetchItems = () => {
    fetch('https://api.themoviedb.org/3/tv/airing_today?api_key=92cd1c00191d7a87cc773c5ee643696c')
      .then(res => {
        if(!res.ok)
        {
  ...
  if(errorItems)
  {
    return <div className="text-white">Error TV Series: {errorItems}</div>
  }
      ...
      {items.slice(0,10).map((item, index) => {
        return (
          ...
          <img src={`https://image.tmdb.org/t/p/original${item.poster_path}`} className="rounded-lg mb-1"/>
            <h2 className="line-clamp-1 text-lg">{item.name}</h2>
            <p className="text-sm text-slate-400 font-light">{handleDateToYear(item.first_air_date)}</p>
