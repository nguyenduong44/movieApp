        ...
        index > 0 && (
          ...
          >
            <LazyLoadImage src={`https://image.tmdb.org/t/p/w342${season.poster_path}`}
              className="h-full inline object-contain rounded-md"
            />
            <div>
              <h1 className="text-xl font-bold mb-2">{season.name}</h1>
              <h1 className="text-sm text-slate-300">Total Episode: {season.episode_count}</h1>
            </div>
          </div>
        )
      ))}
